...
<div class="comm:space-y-2" x-data="{ wasFocused: false }">
    @if (Config::resolveAuthenticatedUser()?->can('create', Config::getCommentModel()))
        <form wire:submit.prevent="save" x-cloak>
            ...
            <template x-if="wasFocused">
                <div class="comm:mb-4">
                    <div class="comm:flex comm:items-center comm:gap-2 comm:mb-2">
                        <input type="file" wire:model="attachments" multiple class="comm:hidden"
                            id="comment-attachments" max="5">
                        <label for="comment-attachments"
                            ...
                            <svg class="comm:w-4 comm:h-4 comm:mr-2" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                    ...
                    @if ($attachments)
                        <div class="comm:space-y-2">
                            @foreach ($attachments as $index => $attachment)
                                <div
                                    class="comm:flex comm:items-center comm:gap-3 comm:p-3 comm:bg-gray-50 comm:rounded-md comm:border comm:border-gray-200">
                                    <div class="comm:flex comm:items-center comm:space-x-2 comm:flex-1 comm:min-w-0">
                                        ...
                                        <div class="comm:flex-1 comm:min-w-0">
                                            <div class="comm:text-sm comm:text-gray-700 comm:truncate"
            ...
            <template x-if="wasFocused">
                <div>
                    <x-filament::button wire:click="save" size="sm">Comment</x-filament::button>
